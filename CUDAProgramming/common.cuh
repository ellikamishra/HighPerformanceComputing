...
// This helper performs C(mxn) = A(m x k) * B(k x n) for row-major inputs on device.
inline void cublas_rowmajor_dgemm(cublasHandle_t h,
  ...
  // We want RowMajor: C = A*B
  // Equivalent ColumnMajor call: C^T = B^T * A^T
  const double *Acol = dB; // note swap
