...
void attention_direct(const double* Q, const double* K, const double* V,
    ...
    for (int i = 0; i < L; i++) {
        ...
        const double* Qi = Q + (std::size_t)i * D;

        for (int j = 0; j < L; ++j) {
            const double* Kj = K + (std::size_t)j * D;
            double dot = 0.0;
            // vectorize inner dot-product across D
            #pragma omp simd reduction(+:dot)
            for (int d = 0; d < D; ++d) dot += Qi[d] * Kj[d];
            s[j] = scale * dot;
        }

        // 2) stable softmax on s
        double mx = s[0];
        ...
        for (int j = 0; j < L; ++j) {
            const double w = s[j];
            const double* Vj = V + (std::size_t)j * D;
            // vectorize accumulation across D
            #pragma omp simd
